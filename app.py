...
# Function to create a conversational chain that can handle both Q&A and summarization
def get_conversational_chain(user_question):
    try:
        # Define a generic Q&A prompt template
        qa_prompt_template = """
        ...
        # Define a summarization-specific prompt template
        summarization_prompt_template = """
        Provide a concise and informative summary of the following context. Focus on key points and main ideas:\n\n
        Context:\n{context}\n
        Summary:
        """

        # Check if the question is likely a summarization request
        is_summarization = any(keyword in user_question.lower() for keyword in ["summarize", "summary", "summarisation"])

        # Select the appropriate prompt and chain type
        if is_summarization:
        ...
        # Create a prompt template based on the selected type
        prompt = PromptTemplate(template=prompt_template, input_variables=["context", "question" if not is_summarization else ""])

        # Load the QA chain with the selected prompt
